...
| **`bsi:component:descriptionLevel`**<br />`optional` | A flag indicating the description level of the component; possible values are `referenced component`, `identified component` and `fully described component`<br />Only valid for use in `components[]/properties` and `metadata/component/properties`. |
